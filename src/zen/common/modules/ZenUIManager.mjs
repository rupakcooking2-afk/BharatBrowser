...
window.gZenUIManager = {
  ...
  init() {
    ...
    // Initialize Bharat Workspace Manager (must be first)
    const { nsBharatWorkspaceManager } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatWorkspaceManager.mjs"
    );
    const { nsBharatWorkspaceDB } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatWorkspaceDB.mjs"
    );
    const { nsBharatWorkspaceMigration } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatWorkspaceMigration.mjs"
    );
    gBharatWorkspaceManager.init();

    // Initialize Bharat Workspace AI Launcher
    const { nsBharatWorkspaceLauncher } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatWorkspaceLauncher.mjs"
    );
    window.gBharatWorkspaceLauncher =
      new nsBharatWorkspaceLauncher();
    window.gBharatWorkspaceLauncher.init();

    // Initialize Bharat AI Sidebar
    const { nsBharatAISidebar } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatAISidebar.mjs"
    );
    gBharatAISidebar.init();

    // Initialize PDF Intelligence
    const { nsPDFIntelligenceManager } = ChromeUtils.importESModule(
      "chrome://browser/content/PDFIntelligenceManager.mjs"
    );
    const { nsPDFTextExtractor } = ChromeUtils.importESModule(
      "chrome://browser/content/PDFTextExtractor.mjs"
    );
    const { nsPDFContextBuilder } = ChromeUtils.importESModule(
      "chrome://browser/content/PDFContextBuilder.mjs"
    );
    const { nsPDFSearchIndex } = ChromeUtils.importESModule(
      "chrome://browser/content/PDFSearchIndex.mjs"
    );
    const { nsPDFSessionStore } = ChromeUtils.importESModule(
      "chrome://browser/content/PDFSessionStore.mjs"
    );
    const { nsPDFOCRProvider } = ChromeUtils.importESModule(
      "chrome://browser/content/PDFOCRProvider.mjs"
    );
    gPDFIntelligenceManager.init();

    const { nsBharatSearchManager } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatSearchManager.mjs"
    );
    gBharatSearchManager.init();

    const { nsBharatChatManager } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatChatManager.mjs"
    );
    gBharatChatManager.init();

    const { nsPageIntelligenceManager } = ChromeUtils.importESModule(
      "chrome://browser/content/PageIntelligenceManager.mjs"
    );
    gPageIntelligenceManager.init();

    const { nsYouTubeIntelligenceManager } = ChromeUtils.importESModule(
      "chrome://browser/content/YouTubeIntelligenceManager.mjs"
    );
    gYouTubeIntelligenceManager.init();

    const { nsBharatNotesManager } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatNotesManager.mjs"
    );
    gBharatNotesManager.init();

    const { nsBharatMemoryManager } = ChromeUtils.importESModule(
      "chrome://browser/content/MemoryManager.mjs"
    );
    const { nsBharatMemorySearch } = ChromeUtils.importESModule(
      "chrome://browser/content/MemorySearch.mjs"
    );
    const { nsBharatMemoryClassifier } = ChromeUtils.importESModule(
      "chrome://browser/content/MemoryClassifier.mjs"
    );
    gBharatMemoryManager.init();

    const { nsBharatResearchManager } = ChromeUtils.importESModule(
      "chrome://browser/content/ResearchManager.mjs"
    );
    const { nsBharatResearchPlanner } = ChromeUtils.importESModule(
      "chrome://browser/content/ResearchPlanner.mjs"
    );
    const { nsBharatResearchExecutor } = ChromeUtils.importESModule(
      "chrome://browser/content/ResearchExecutor.mjs"
    );
    const { nsBharatResearchReportBuilder } = ChromeUtils.importESModule(
      "chrome://browser/content/ResearchReportBuilder.mjs"
    );
    const { nsBharatResearchCitationEngine } = ChromeUtils.importESModule(
      "chrome://browser/content/ResearchCitationEngine.mjs"
    );
    const { nsBharatResearchMemoryBridge } = ChromeUtils.importESModule(
      "chrome://browser/content/ResearchMemoryBridge.mjs"
    );
    gBharatResearchManager.init();

    // Initialize Voice Assistant
    const { nsVoiceManager } = ChromeUtils.importESModule(
      "chrome://browser/content/VoiceManager.mjs"
    );
    gVoiceManager.init();

    // Initialize Bharat Workspace AI Launcher (dedicated window-based launcher)
    const { nsBharatWorkspaceAILauncher } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatWorkspaceAILauncher.mjs"
    );
    window.gBharatWorkspaceAILauncher.init();

    // Initialize Bharat Browser AI Core (MARK XXXIX → browser-native OS)
    const { nsBharatAICore } = ChromeUtils.importESModule(
      "chrome://browser/content/BharatAICore.mjs"
    );
    ChromeUtils.importESModule("chrome://browser/content/BrowserContextBus.mjs");
    ChromeUtils.importESModule("chrome://browser/content/BrowserActionApproval.mjs");
    ChromeUtils.importESModule("chrome://browser/content/BrowserActionRegistry.mjs");
    ChromeUtils.importESModule("chrome://browser/content/BharatAgentPlanner.mjs");
    ChromeUtils.importESModule("chrome://browser/content/BharatAgentExecutor.mjs");
    ChromeUtils.importESModule("chrome://browser/content/TabIntelligenceManager.mjs");
    ChromeUtils.importESModule("chrome://browser/content/AgentTaskManager.mjs");
    ChromeUtils.importESModule("chrome://browser/content/AgentMemoryBridge.mjs");
    ChromeUtils.importESModule("chrome://browser/content/AgentAuditLog.mjs");
    gBharatAICore.init();

    // Run workspace migration after all managers are initialized
    // (migration needs access to Notes/Memory DBs which are initialized above)
    setTimeout(() => {
      nsBharatWorkspaceMigration.run();
    }, 0);
